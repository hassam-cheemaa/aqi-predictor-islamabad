...
def run():
    ...
    # Get all data and drop rows where target is missing (since we shifted by 72 hours)
    df = feature_view.get_batch_data()
    df = df.dropna(subset=["target_aqi_next_3_days"])
    
    # Features and Target
    X = df.drop(columns=["target_aqi_next_3_days", "timestamp"])
    ...
    print(f"Model Evaluation - RMSE: {rmse:.4f}, MAE: {mae:.4f}, R2: {r2:.4f}")
    ...
    print("Registering model to Hopsworks...")
