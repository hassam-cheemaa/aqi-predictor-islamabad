...
try:
    ...
    if current_aqi <= 50:
        ...
        st.error(f"Current AQI: {current_aqi} (Unhealthy for Sensitive Groups)")
    else:
        st.error(f"⚠️ HAZARDOUS Current AQI: {current_aqi} ⚠️")
        
    # Prepare features for prediction
    X_pred = latest_data.drop(columns=["target_aqi_next_3_days", "timestamp"], errors='ignore')
    
    predicted_aqi = model.predict(X_pred)[0]
    
    st.header("Forecast: AQI in 3 Days")
    st.metric(label="Predicted AQI", value=f"{predicted_aqi:.2f}", delta=f"{predicted_aqi - current_aqi:.2f} from now", delta_color="inverse")
    
    if predicted_aqi > 150:
        st.error("⚠️ ALERT: The predicted AQI is expected to reach hazardous levels in 3 days. Please take precautions.")
    ...
    shap.waterfall_plot(shap.Explanation(values=shap_values[0], 
                                       base_values=explainer.expected_value[0] if isinstance(explainer.expected_value, (list, tuple)) else explainer.expected_value, 
                                       data=X_pred.iloc[0], 
    ...
    st.write("Make sure you have run the feature_pipeline.py and training_pipeline.py scripts and configured your Hopsworks API key.")
